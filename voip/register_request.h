#ifndef VOIP_REGISTER_REQUEST_H
#define VOIP_REGISTER_REQUEST_H
...
{
public:
    explicit RegisterRequest(std::string host, std::string port, std::string target)
    ...
    // 发送请求（带超时与异常处理）
    std::string Request(const std::string &client_Id, const std::string &customer_info, const std::string &call_method, const std::string &call_type, int timeout_seconds = 5)
    {
        try {
