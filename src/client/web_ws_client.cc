...
#include "../core/check_acc.h"
#include "../core/check_acc_mgr.h"
...
WebWsClient::WebWsClient()
    : m_thread_pool(5)
    , m_caller_que(std::make_shared<OutcomingCallQue>())
    ...
    m_on_read_handler = [this](const std::string &msg) {
        ...
        if (request_type == "check") {
            ...
            if (call_type == "single") {
                ...
                auto caller = std::make_shared<OutcomingCall>(*acc);
...
void WebWsClient::single_call(std::shared_ptr<Coordinator> coordinator)
{
    ...
    caller->makeSingleCall(dialplan, m_call_method, m_different, coordinator);
...
void WebWsClient::group_call(std::size_t i, std::shared_ptr<Coordinator> coordinator)
{
    ...
    auto caller = m_caller_que->getCaller();
    auto dialplan = m_dialplan_que->getDialPlan();
    caller->makeGroupCall(dialplan, m_call_method, m_different, coordinator);
    LOG_INFO("call {} over", dialplan);
}
